...
class HomeController < ApplicationController
	...
	def map
		...
		if $view == "title" and params[:button] == "start"
            ...
            strong_monarr.each{|idx| monarr.push(idx)}
		    ...
		    $selected = Management.new allmon
            puts "====GET_MOVIE_TEST===="
            puts allmon[0]['name']
            puts $selected.get_movie allmon[0]['name']
            puts "----------------------"
            puts allmon[1]['name']
            puts $selected.get_movie allmon[1]['name']
            puts "----------------------"

            if rand(2) == 0
                allmon.pop(strong_monarr.length)
				$game = allmon
            else
                $game = allmon.pop(strong_monarr.length)
			end
			puts "========================="
			puts $game.length
			puts $game
